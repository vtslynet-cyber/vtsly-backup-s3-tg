...
## ♻️ Стабильность и автозапуск

Скрипт рассчитан на постоянную работу без вмешательства:

- **Переживает перезагрузку сервера.** Конфиг, команда и задача cron лежат на диске; демон cron включается в автозапуск (systemd / OpenRC / sysvinit) — после ребута расписание поднимается само, перенастраивать ничего не нужно.
- **Догоняющий бэкап после простоя.** Помимо обычного расписания добавляется `@reboot`-задача: если сервер был выключен в момент планового бэкапа, копия будет сделана через ~2 минуты после загрузки. Так пропуск из-за простоя компенсируется.
